...
function PlayerTeamList({ user }) {
  ...
  useEffect(() => {
    fetch(`/teams/${id}`)
      ...
      .catch((error) =>
        console.error(`There was an error fetching team at id: ${id}: ${error}`)
      );
  }, [id]);
      ...
      <div>
        <div className="teamsHeaders">
          ...
          <h3>
            {team_name} -{" "}
            <span id="age-group">
              {gender === "F" ? `G${age_group}` : age_group}
            </span>
          </h3>
              ...
              <tbody>
                {coaches.map((coach) => {
                  return (
                    <tr key={coach.id}>
                      <td>
                        {coach.headshot_img_url ? (
                          <img
                            src={coach.headshot_img_url}
                            alt={`${coach.coach_name} Headshot`}
                            style={{
                              width: "55px",
                              height: "60px",
                              borderRadius: "5px",
                  ...
                  {players.map((player) => {
                    ...
                    return (
                      <tr key={player.id}>
                        ...
                        <td>
                          {player.headshot_img_url ? (
                            <img
                              src={player.headshot_img_url}
