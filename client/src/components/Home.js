...
function Home({events, user, teams, handleDeleteEvent, handleShowSuccessfulDeleteAlert, handleShowErrorDeleteAlert, showSuccessfulDeleteAlert, showErrorDeleteAlert, newsArticle}) {
  ...
  if (newsArticle === undefined) {
    ...
    const formattedDate = dayjs(newsArticle.publishedAt.slice(0,10)).format('MM/DD/YYYY')
    ...
    return (
      <div className="right">
        ...
        <div id="sports-news-container" style={{backgroundImage: `url(${newsArticle.urlToImage})`}}>
          {newsArticle.urlToImage? null : <Image
            width={500}
            height={500}
          ...
          />}
          <div id="news-text">
            <p style={newsStyles}>{newsArticle.source.name}</p>
            <p id="article-title">{newsArticle.title}</p>              
            <p>{newsArticle.description}</p>
            <p style={newsStyles}>{newsArticle.author? newsArticle.author : "N/A"} - {formattedDate}</p>
            <a id="read-more" href={newsArticle.url} target="_blank" rel="noopener noreferrer">
            READ MORE
            </a>
          </div>
        </div>
