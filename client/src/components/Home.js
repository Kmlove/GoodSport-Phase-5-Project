import ScheduleCard from "./ScheduleCard";
import { Alert, Image } from "antd";
import { Link } from "react-router-dom";
import "../CSS/homeStyles.css";
import dayjs from "dayjs";
import { SportsNewsContext } from "../context/sportsNews";
import { useContext } from "react";

function Home({
  events,
  user,
  teams,
  handleDeleteEvent,
  handleShowSuccessfulDeleteAlert,
  handleShowErrorDeleteAlert,
  showSuccessfulDeleteAlert,
  showErrorDeleteAlert,
}) {
  const newsStyles = {
    width: "fit-content",
    backgroundColor: "#6e41a1",
    padding: "8px",
    borderRadius: "5px",
  };

  const newsArticle = useContext(SportsNewsContext);

  const today = new Date();
  const day =
    today.getDate().toString().length === 1
      ? `0${today.getDate()}`
      : today.getDate();
  const month =
    (today.getMonth() + 1).toString().length === 1
      ? `0${today.getMonth() + 1}`
      : today.getMonth() + 1;
  const todaysDate = `${today.getFullYear()}-${month}-${day}`;
  const upcomingEvents = events
    .filter((event) => {
      const eventYear = event.date.slice(0, 4);
      const currYear = today.getFullYear().toString();
      return eventYear >= currYear;
    })
    .filter((event) => {
      const eventYear = event.date.slice(0, 4);
      console.log("event year", eventYear);
      const currYear = today.getFullYear().toString();
      if (eventYear > currYear) {
        return eventYear > currYear;
      } else {
        const eventMonth = event.date.slice(5, 7);
        const currMonth = todaysDate.slice(5, 7);
        return eventMonth >= currMonth;
      }
    })
    .filter((event) => {
      const eventYear = event.date.slice(0, 4);
      const currYear = today.getFullYear().toString();
      if (eventYear > currYear) {
        return eventYear > currYear;
      } else {
        const eventDay = event.date.slice(8, 10);
        const currDay = todaysDate.slice(8, 10);
        return eventDay >= currDay;
      }
    });
  const homePageEvents = upcomingEvents.slice(0, 3);

  if (newsArticle === undefined) {
    return (
      <div className="right">
        <h2 className="containerHeaders">Home</h2>
        <h3>Loading...</h3>
      </div>
    );
  } else if (events.length === 0 || homePageEvents.length === 0) {
    const formattedDate = dayjs(newsArticle.pubDate.slice(0, 10)).format(
      "MM/DD/YYYY"
    );

    return (
      <div className="right">
        <h2 className="containerHeaders">Home</h2>
        <h3 id="upcoming-events">Sports News:</h3>

        <div
          id="sports-news-container"
          style={{ backgroundImage: `url(${newsArticle.image_url})` }}
        >
          {newsArticle.image_url ? null : (
            <Image
              width={500}
              height={500}
              style={{ display: "block", margin: "0 auto" }}
              src="error"
              preview={false}
              alt={`${newsArticle.title}`}
              fallback="data:image/png;base64,"
            />
          )}
          <div id="news-text">
            <p style={newsStyles}>{newsArticle.source_id}</p>
            <p id="article-title">{newsArticle.title}</p>
            <p>
              {newsArticle.description
                ? newsArticle.description.slice(0, 100) + "..."
                : newsArticle.content.slice(0, 100) + "..."}
            </p>
            <p style={newsStyles}>
              {newsArticle.creator ? newsArticle.creator[0] : "N/A"} -{" "}
              {formattedDate}
            </p>
            <a
              id="read-more"
              href={newsArticle.link}
              target="_blank"
              rel="noopener noreferrer"
            >
              READ MORE
            </a>
          </div>
        </div>

        <h3 id="upcoming-events">Upcoming Events:</h3>
        <h4 className="no-upcoming">You Have No Upcoming Events...</h4>
      </div>
    );
  } else {
    const formattedDate = dayjs(newsArticle.pubDate.slice(0, 10)).format(
      "MM/DD/YYYY"
    );

    return (
      <div className="right">
        <h2 className="containerHeaders">Home</h2>
        <h3 id="upcoming-events">Sports News:</h3>

        <div
          id="sports-news-container"
          style={{ backgroundImage: `url(${newsArticle.image_url})` }}
        >
          {newsArticle.image_url ? null : (
            <Image
              width={500}
              height={500}
              style={{ display: "block", margin: "0 auto" }}
              src="error"
              preview={false}
              alt={`${newsArticle.title}`}
              fallback="data:image/png;base64,"
            />
          )}
          <div id="news-text">
            <p style={newsStyles}>{newsArticle.source_id}</p>
            <p id="article-title">{newsArticle.title}</p>
            <p>
              {newsArticle.description
                ? newsArticle.description.slice(0, 100) + "..."
                : newsArticle.content.slice(0, 100) + "..."}
            </p>
            <p style={newsStyles}>
              {newsArticle.creator ? newsArticle.creator[0] : "N/A"} -{" "}
              {formattedDate}
            </p>
            <a
              id="read-more"
              href={newsArticle.link}
              target="_blank"
              rel="noopener noreferrer"
            >
              READ MORE
            </a>
          </div>
        </div>

        <h3 id="upcoming-events">
          Upcoming Events:{" "}
          <Link to={"/schedule"} id="see-full-schedule">
            See full schedule...
          </Link>
        </h3>

        {showSuccessfulDeleteAlert ? (
          <Alert
            className="alert-top"
            message="Event Successfully Deleted"
            type="success"
            banner
            showIcon
          />
        ) : null}
        {showErrorDeleteAlert ? (
          <Alert
            className="alert-top"
            message="An error occured deleting the event, please try again later!"
            type="error"
            banner
            showIcon
          />
        ) : null}

        <div id="home-page-schedule">
          {homePageEvents.map((event) => (
            <ScheduleCard
              key={event.id}
              event={event}
              user={user}
              teams={teams}
              handleDeleteEvent={handleDeleteEvent}
              handleShowSuccessfulDeleteAlert={handleShowSuccessfulDeleteAlert}
              handleShowErrorDeleteAlert={handleShowErrorDeleteAlert}
            />
          ))}
        </div>
      </div>
    );
  }
}

export default Home;
